...
class TabPFNWrapper(FoundationModelWrapper):
    ...
    def _fit_model(
        self, X: pd.DataFrame, forecast_columns: ForecastColumnConfig, freq: Frequency
    ):
        """
        ...
        logging.info(
            f"Training TabPFN with {len(X_train)} samples and {self.n_features} features"
        )

        # Handle sample size limitation
        if len(X_train) > self.max_samples:
            logging.info(
                f"Taking most recent {self.max_samples} from {len(X_train)} samples for TabPFN"
            )
    ...
    def _forecast_series(
        ...
        if len(target_values) < self.n_lags:
            ...
            return np.full(
                horizon, target_values[-1] if len(target_values) > 0 else 0.0
            )
    ...
    def _add_time_delta(
        self, date: pd.Timestamp, steps: int, freq: Frequency
    ) -> pd.Timestamp:
