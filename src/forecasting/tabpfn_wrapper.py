...
class TabPFNWrapper(BaseEstimator, RegressorMixin, FoundationModelWrapper):
    ...
    supports_exogenous = False  # TabPFN doesn't support exogenous features in time series context
    ...
    def fit(
        ...
        **fit_params : dict
        ...
        if len(X_data) > self.max_samples:
            ...
            indices = np.random.choice(len(X_data), size=self.max_samples, replace=False)
    ...
    def set_params(self, **params):
        ...
        for key, value in params.items():
            if key in ['device', 'n_estimators', 'max_samples', 'random_state', 'scaling']:
